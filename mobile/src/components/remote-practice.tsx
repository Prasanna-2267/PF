...
function PracticeRunner({ initial, onExit }: { initial: PracticeSession; onExit: () => void }) {
  ...
  const submit = useMutation({
    ...
    onSuccess: async ({ data, durationMs, questionId, sequence }) => {
      ...
      await Promise.all([questions.refetch(), session.refetch(), qc.invalidateQueries({ queryKey: ['student', 'practice', 'modes'], refetchType: 'none' }), qc.invalidateQueries({ queryKey: ['student', 'practice', 'search'], refetchType: 'none' })]);
    },
  ...
  const finish = useMutation({
    mutationFn: async () => {
      const completed = await completePracticeSession(initial.id);
      return { result: await getPracticeResult(initial.id), completionTimingReport: completed.completionTimingReport };
    },
    onSuccess: async ({ result: completedResult, completionTimingReport }) => {
      if (untimed && completionTimingReport) setQuestionTimings(completionTimingReport);
      setResult(completedResult);
      await qc.invalidateQueries({ queryKey: ['student', 'practice'] });
    },
    onError: resumeQuestionStopwatch,
  });
  const goToQuestion = (next: number) => { pauseQuestionStopwatch(); setIndex(next); setSelected(undefined); setFeedback(undefined); };
