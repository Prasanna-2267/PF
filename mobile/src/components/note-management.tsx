      ...
      <Pressable accessibilityRole="button" accessibilityLabel={`Open ${lesson.title}`} onPress={onOpen} style={({ pressed }) => [styles.main, pressed && styles.pressed]}>
        ...
        <View style={styles.copy}>
          ...
          {accessible || status.read ? <View style={styles.status}>
            ...
            {status.revisions ? <Text numberOfLines={1} style={[styles.revision, { color: theme.goldStrong }]}>{status.revisions}× revised</Text> : null}
            {!accessible ? <Text numberOfLines={1} style={[styles.premiumTracking, { color: theme.goldStrong }]}>Premium locked</Text> : null}
...
export function NoteActionSheet({ lesson, visible, onClose }: { lesson: Lesson; visible: boolean; onClose: () => void }) {
  ...
  return <Modal transparent visible={visible} animationType="none" onRequestClose={onClose}>
    <View style={styles.modalRoot}><Pressable style={styles.scrim} onPress={onClose} /><View style={[styles.sheet, { backgroundColor: theme.surface }]}>
      ...
      <Action icon={<Bookmark fill={status.favourite ? theme.goldStrong : 'transparent'} color={theme.goldStrong} size={18} />} iconBackground={theme.goldSoft} title={status.favourite ? 'Remove from favourites' : 'Add to favourites'} detail={accessible ? 'Keep this note in your quick-access list.' : 'Save this premium note to your list without unlocking it.'} border={theme.line} fg={theme.fg} muted={theme.muted} onPress={() => toggleFavourite(lesson.id, initialReaderStatus(lesson))} />
      <Action icon={<RotateCw color={theme.primary} size={18} />} iconBackground={theme.primarySoft} title="Revise this note" detail={status.revisions ? `${status.revisions} revisions logged so far.` : accessible ? 'Log your first revision.' : 'Record a revision while the premium PDF remains locked.'} border={theme.line} fg={theme.fg} muted={theme.muted} onPress={() => revise(lesson.id, initialReaderStatus(lesson))} />
      {!accessible ? <View style={[styles.accessNotice, { backgroundColor: theme.goldSoft }]}><LockKeyhole size={16} color={theme.goldStrong} /><Text style={[styles.accessNoticeText, { color: theme.goldStrong }]}> Purchase this material to view it.</Text></View> : null}
    </View></View>
  </Modal>;
}
