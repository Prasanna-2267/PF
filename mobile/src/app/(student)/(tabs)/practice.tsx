...
import { BookOpen, CheckCircle2, CircleHelp, Clock3, Flag, Layers3, Minus, PenLine, Play, Plus, RotateCcw } from 'lucide-react-native';
...
export default function PracticeScreen() {
  ...
  const clearAttempt = () => { setStarted(false); setCurrent(0); setAnswers({}); setMarked(new Set()); setSubmitted(new Set()); setWritten(''); setWrittenSubmitted(false); setTimeLeft(timerDuration ?? 0); };
  ...
  const startSession = () => { if (mode === 'mcq' && timerDuration !== null && timerDuration <= 0) return; setCurrent(0); setAnswers({}); setMarked(new Set()); setSubmitted(new Set()); setWritten(''); setWrittenSubmitted(false); setTimeLeft(timerDuration ?? 0); setStarted(true); };
  const selectAnswer = (option: string) => { if (!isSubmitted && (timerDuration === null || timeLeft > 0)) setAnswers((value) => ({ ...value, [question.id]: option })); };
  ...
  return <SafeAreaView edges={['top', 'left', 'right']} style={[styles.safe, { backgroundColor: theme.canvas }]}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    <View><Text style={[styles.eyebrow, { color: theme.primary }]}>FOCUSED PRACTICE</Text><Text style={[styles.title, { color: theme.fg }]}>Practice</Text><Text style={[styles.description, { color: theme.muted }]}>Choose a subject and topic before starting. Every question in the set will stay within that topic.</Text></View>
