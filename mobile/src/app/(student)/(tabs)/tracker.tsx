...
function ConsistencyChart({ remote }: { remote?: TrackerSummary }) {
  const { theme } = useAppTheme();
  const chartWeek = remote ? remote.consistency.slice(-7).map((entry) => ({ day: new Date(`${entry.date}T12:00:00`).toLocaleDateString('en-US', { weekday: 'narrow' }), minutes: Math.round(entry.totalSeconds / 60) })) : week;
  const totalMinutes = remote ? Math.round(remote.summary.totalSeconds / 60) : chartWeek.reduce((sum, entry) => sum + entry.minutes, 0);
  const maxMinutes = Math.max(180, ...chartWeek.map((entry) => entry.minutes), remote?.consistency.at(-1)?.targetMinutes ?? 0);
  const change = remote?.summary.changePercent ?? 12;
  const bestMinutes = remote?.summary.bestDay ? Math.round(remote.summary.bestDay.totalSeconds / 60) : 164;
  return <Card style={styles.chartCard}>
    <View style={styles.chartHeading}><View><Text style={[styles.sectionEyebrow, { color: theme.primary }]}>LAST 7 DAYS</Text><Text style={[styles.cardTitle, { color: theme.fg }]}>Study consistency</Text><Text style={[styles.chartSummary, { color: theme.muted }]}>{formatMinutes(totalMinutes)} total · {formatMinutes(remote ? Math.round(remote.summary.dailyAverageSeconds / 60) : 102)} daily average</Text></View><View style={[styles.trendBadge, { backgroundColor: change >= 0 ? theme.successSoft : theme.goldSoft }]}><TrendingUp color={change >= 0 ? theme.success : theme.goldStrong} size={15} /><Text style={[styles.trendText, { color: change >= 0 ? theme.success : theme.goldStrong }]}>{change > 0 ? '+' : ''}{change}%</Text></View></View>
    <View style={styles.chartArea}>
      <View style={[styles.goalLine, { borderTopColor: theme.lineStrong }]}><Text style={[styles.goalLabel, { color: theme.faint }]}>{formatMinutes(remote?.consistency.at(-1)?.targetMinutes ?? 120)} goal</Text></View>
      <View style={styles.bars}>{chartWeek.map((entry, index) => { const height = Math.max(10, Math.round((entry.minutes / maxMinutes) * 104)); const today = index === chartWeek.length - 1; return <View key={`${entry.day}-${index}`} style={styles.barColumn}><Text style={[styles.barValue, { color: today ? theme.primaryStrong : theme.muted }]}>{Math.round(entry.minutes / 6) / 10}h</Text><View style={[styles.barTrack, { backgroundColor: theme.sunken }]}><LinearGradient colors={today ? [theme.primaryStrong, theme.primary] : [theme.lineStrong, theme.primarySoft]} style={[styles.barFill, { height }]} /></View><Text style={[styles.dayLabel, { color: today ? theme.primaryStrong : theme.muted }]}>{entry.day}</Text>{today ? <View style={[styles.todayDot, { backgroundColor: theme.primary }]} /> : null}</View>; })}</View>
    </View>
    <View style={styles.chartInsights}><Insight icon={<Target color={theme.primary} size={16} />} value={`${remote?.summary.goalDays ?? 5}/7`} label="Goal days" /><View style={[styles.insightDivider, { backgroundColor: theme.line }]} /><Insight icon={<Flame color={theme.goldStrong} size={16} />} value={formatMinutes(bestMinutes)} label="Best day" /><View style={[styles.insightDivider, { backgroundColor: theme.line }]} /><Insight icon={<CalendarClock color={theme.success} size={16} />} value={`${remote?.streak.current ?? 6} days`} label="Current streak" /></View>
  </Card>;
}
...
export default function TrackerScreen() {
  const { theme } = useAppTheme(); const dark = theme.canvas === themes.dark.canvas; const study = useStudySession('TRACKER', demoStudy.targetMinutes);
  const demo = isDemoSession();
  const tracker = useQuery({ queryKey: ['student', 'tracker', 'summary', 7], queryFn: () => getTrackerSummary(7), enabled: !demo });
  ...
  const examPressure = trackerData?.exam.pressurePercent ?? demoStudy.exam.pressure;
  const examDate = trackerData?.exam.date ? new Date(trackerData.exam.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : demoStudy.exam.date;
  return <SafeAreaView edges={['top', 'left', 'right']} style={[styles.safe, { backgroundColor: theme.canvas }]}><ScrollView contentContainerStyle={[styles.content, wide && styles.contentWide]} showsVerticalScrollIndicator={false}>
    ...
    <LinearGradient colors={dark ? ['#121A2B', '#10151D', '#0D1015'] : ['#F1F5FF', '#F8FAFF', '#FFFFFF']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.readinessHub, { borderColor: theme.line }]}>
      ...
      <View style={[styles.readinessHubBody, wide && styles.readinessHubBodyWide]}>
        <View style={[styles.responsiveCell, wide && styles.responsiveCellWide]}><View style={styles.readiness}><ReadinessCard icon={<BookCheck size={18} color={theme.primaryStrong} />} label="SYLLABUS" value={`${trackerData?.readiness.syllabusPercent ?? demoStudy.syllabusPercent}%`} detail={trackerData ? `${trackerData.readiness.completedNotes}/${trackerData.readiness.totalNotes} notes completed` : 'Coverage completed'} progress={trackerData?.readiness.syllabusPercent ?? demoStudy.syllabusPercent} color={theme.primaryStrong} /><ReadinessCard icon={<RotateCcw size={18} color={theme.goldStrong} />} label="REVISIONS" value={`${trackerData?.readiness.revisions ?? demoStudy.revisions}`} detail="Intentional returns" progress={trackerData ? Math.min(100, trackerData.readiness.revisions * 10) : 60} color={theme.goldStrong} /></View></View>
