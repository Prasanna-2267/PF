...
import { ArrowLeft, ChevronRight, ExternalLink, FileImage, FileText, Folder, LockKeyhole, PackageCheck, ShoppingBag } from 'lucide-react-native';
...
export default function PackageDetailScreen() {
  ...
  const browseStore = async () => {
    if (!item) return;
    await WebBrowser.openBrowserAsync(`${STORE_BASE_URL}/store/product/${encodeURIComponent(item.slug || item.id)}`);
  };
  ...
  return <SafeAreaView edges={['top', 'left', 'right']} style={[styles.safe, { backgroundColor: theme.canvas }]}>
    ...
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <View style={[styles.hero, { backgroundColor: theme.surface, borderColor: theme.line }]}>
        ...
        <View style={[styles.priceRow, { borderTopColor: theme.line }]}><View><Text style={[styles.priceLabel, { color: theme.muted }]}>PACKAGE PRICE</Text><Text style={[styles.price, { color: theme.fg }]}>{formatMoney(item.price?.amount)}</Text></View><View style={styles.counts}><Text style={[styles.count, { color: theme.fg }]}>{item.itemCount}</Text><Text style={[styles.countLabel, { color: theme.muted }]}>files</Text></View><View style={styles.counts}><Text style={[styles.count, { color: theme.fg }]}>{item.folderCount ?? 0}</Text><Text style={[styles.countLabel, { color: theme.muted }]}>folders</Text></View></View>
      </View>
      ...
      <View style={styles.sectionHead}><View><Text style={[styles.sectionTitle, { color: theme.fg }]}>What’s inside</Text><Text style={[styles.sectionCopy, { color: theme.muted }]}>Complete package structure</Text></View><Text style={[styles.total, { color: theme.muted }]}>{item.itemCount} files</Text></View>
...
function formatMoney(amount?: number) { return amount === undefined ? 'Price unavailable' : amount === 0 ? 'Free' : `₹${amount.toLocaleString('en-IN')}`; }
